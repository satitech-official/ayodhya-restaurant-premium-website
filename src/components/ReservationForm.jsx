...
function reservationWhatsAppUrl(form) {
  const phone = RESTAURANT.whatsappHref.replace(/\/$/, "");
  const message = [
    "Hello Ayodhya Restaurant, I would like to request a table reservation.",
    "",
    `Name: ${form.name}`,
    `Phone: ${form.phone}`,
    `Date: ${form.date}`,
    ...
    <form onSubmit={submit} className="rounded-[2rem] bg-soft p-6 shadow-soft ring-1 ring-sand/60 sm:p-8">
      <div className={compact ? "grid gap-4 sm:grid-cols-2" : "grid gap-4 sm:grid-cols-2"}>
        <div className={compact ? "" : "sm:col-span-2"}>
          ...
          <input id="res-name" required value={form.name} onChange={set("name")} placeholder="Your name" className={inputCls} />
        </div>
        <div className={compact ? "" : "sm:col-span-2"}>
          <label htmlFor="res-phone" className={labelCls}>Phone Number</label>
          <input id="res-phone" required type="tel" value={form.phone} onChange={set("phone")} placeholder="+91 …" className={inputCls} />
        </div>
        <div>
          <label htmlFor="res-date" className={labelCls}>Date</label>
          <input id="res-date" required type="date" min={todayISO()} value={form.date} onChange={set("date")} className={inputCls} />
        </div>
        <div>
          <label htmlFor="res-time" className={labelCls}>Time</label>
          <input id="res-time" required type="time" value={form.time} onChange={set("time")} className={inputCls} />
        </div>
        <div>
        ...
        <div className="sm:col-span-2">
          <label htmlFor="res-seating" className={labelCls}>Seating Preference</label>
          <select id="res-seating" value={form.seating} onChange={set("seating")} className={inputCls}>
            {SEATING_PREFERENCES.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        <div className="sm:col-span-2">
          <label htmlFor="res-request" className={labelCls}>Special Request <span className="font-normal normal-case text-walnut/60">(optional)</span></label>
          <textarea id="res-request" rows={3} value={form.specialRequest} onChange={set("specialRequest")} placeholder="Birthday cake, window seat, allergen note…" className={inputCls} />
        </div>
      </div>

      {error && <p className="mt-4 rounded-xl bg-terracotta/10 px-4 py-3 text-sm text-terracotta">{error}</p>}

      <button type="submit" disabled={status === "loading"} className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-terracotta px-7 py-4 text-sm font-bold uppercase tracking-[0.12em] text-soft transition hover:bg-burnt disabled:opacity-60">
        {status === "loading" ? <><Loader2 className="h-4 w-4 animate-spin" /> Submitting…</> : <><Phone className="h-4 w-4" /> Request Reservation</>}
      </button>
      <p className="mt-3 text-center text-xs text-walnut/70">We'll confirm your table by phone or WhatsApp.</p>
    </form>
  );
}
